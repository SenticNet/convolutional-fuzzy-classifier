...
import sys
import os
sys.path.append(os.path.abspath('./morb'))
...
neuron = 10;layer0gW = 0;layer1gW = 0;layer1bgW = 0;
layer1cgW = 0;layer1dgW =0;layer1egW = 0;layer1fgW = 0;
layer1ggW = 0;layer1hgW = 0;layer2gW = 0;layer3gW = 0;
layer0gb = 0;layer1gb = 0;layer1bgb = 0;layer1cgb = 0;
layer1dgb = 0;layer1egb = 0;layer1fgb = 0;layer1ggb = 0;
layer1hgb = 0;layer2gb = 0;layer3gb = 0;all_test = 0;
eval_print1 = 0;eval_print2 = 0;eval_print3 = 0;
epoch_cd = 0; batchm = 0;
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
                    ...
                    nkerns=[5, 5, 5, 5, 5, 5, 5, 5, 5], dirn=".",batch_size=500, indexd=0):
...
# Reshape matrix of rasterized images of shape (batch_size, 28 * 28)
    ...
    im2x = int((im1x-nk1x+1)/poolx)
    im2y = int((im1y-nk1y+1)/pooly)
...
#    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
#    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2bx = int((im2x-nk2x+1)/poolx)
    im2by = int((im2y-nk2y+1)/pooly)
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2cx = int((im2bx-nk2bx+1)/poolx)
    im2cy = int((im2by-nk2by+1)/pooly)
    nk2cx=7
    nk2cy=im2cy
    Wl = None;
    bl = None;
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2dx = int((im2cx-nk2cx+1)/poolx)
    im2dy = int((im2cy-nk2cy+1)/pooly)
    nk2dx=3
    nk2dy=im2dy
    Wl = None;
    bl = None;
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2ex = int((im2dx-nk2dx+1)/poolx)
    im2ey = int((im2dy-nk2dy+1)/pooly)
    nk2ex=2
    nk2ey=im2ey
    Wl = None;
    bl = None;
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2fx = int((im2ex-nk2ex+1)/poolx)
    im2fy = int((im2ey-nk2ey+1)/pooly)
    nk2fx=2
    nk2fy=im2fy
    Wl = None;
    bl = None;
...
#        b = bl
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2gx = int((im2fx-nk2fx+1)/poolx)
    im2gy = int((im2fy-nk2fy+1)/pooly)
    nk2gx=2
    nk2gy=im2gy
    Wl = None;
    bl = None;
    ...
    # maxpooling reduces this further to (8/2, 8/2) = (4, 4)
    # 4D output tensor is thus of shape (batch_size, nkerns[1], 4, 4)
    im2hx = int((im2gx-nk2gx+1)/poolx)
    im2hy = int((im2gy-nk2gy+1)/pooly)
    nk2hx=2
    nk2hy=im2hy
    Wl = None;
    bl = None;
    ...
    im3x = int((im2hx-nk2hx+1)/poolx)
    im3y = int((im2hy-nk2hy+1)/pooly)
    Wl = None;
    bl = None;
    ...
    layer3 = LogisticRegression(input=layer2.output, n_in=50, n_out=4, W = Wl, b = bl)
...
# create a list of all model parameters to be fit by gradient descent
    ...
    try:
     predy = open(dirgtest+"/run2/pred_y"+str(indexd)+".csv",'r+b')
    except IOError:
     predy = open(dirgtest+"/run2/pred_y"+str(indexd)+".csv",'wb')
    ...
    np.savetxt(predy,test_losses,fmt='%u');
    ...
    if indexd == indexd: 
        ...
        myfile = open("run2/layer0_vid_test"+str(indexd)+".csv",'w');
        ...
        myfile = open("run2/layer0_vid_train"+str(indexd)+".csv",'w');
        wr = csv.writer(myfile, quotechar=None,escapechar='\\')
        wr.writerows(eval_print)
        myfile.close()
        ...
        myfile = open("run2/layer0_vid_val"+str(indexd)+".csv",'w');
        wr = csv.writer(myfile, quotechar=None,escapechar='\\')
        wr.writerows(eval_print)
        myfile.close()

    if indexd == indexd:
        with open(dirgtest+"/weights/layer0w_"+str(indexd)+".save", 'wb') as f:
           cPickle.dump(layer0.W.get_value(), f, protocol=cPickle.HIGHEST_PROTOCOL)
        f.close()

        with open(dirgtest+"/weights/layer0b_"+str(indexd)+".save", 'wb') as f:
           cPickle.dump(layer0.b.get_value(), f, protocol=cPickle.HIGHEST_PROTOCOL)
        f.close()

    if indexd == indexd:
        myfile = open(dirgtest+"/weights/layer0w_"+str(indexd)+".csv",'w');
        wr = csv.writer(myfile, quotechar=None,escapechar='\\')
        wr.writerows(layer0.W.get_value())
        myfile.close()
...
if __name__ == '__main__':
    ...
    for i in range(1):
          ...
          evaluate_lenet5(dataset=nameg, n_epochs = 300, dirn=dirgtest, indexd=k)
