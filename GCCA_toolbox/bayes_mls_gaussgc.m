function [test_set_predictions, chain_stats, final_pred] = bayes_mls_gaussgc(data,test,options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if nargin==3 % if then using user defined options
   ...
   mcmc_samples = 50; options.mcmc_samples = mcmc_samples; % see below
   burn_in = 50; options.burn_in =burn_in; % see below
...
while sample < mcmc_samples
   ...
   if count>burn_in
      ...
      if test_er < 0.01 && sample > 2*burn_in
%          sample = mcmc_samples + 1;
      end
...
loopcnt2 = 0;
while not_finished && loopcnt2 < 20
