...
for ii=1:nvar
    ...
    d1 = 0:1;
    d2 = 0:1;
    net = newdtdnn(xd,yd,size(xd,1),{d1,d2});
...
% 
%     %yd = datamvu(1,:);
%     %xd = datamvu(2:size(datamvu,1),:);
% 
%     yd = [datamvu(1,:) zeros(1,nobs-size(datamvu,2))+0.01];
%     xd = [datamvu(2:size(datamvu,1),:) zeros(size(datamvu,1)-1,nobs-size(datamvu,2))+0.01];
%     
%     cd E:\GCCA_toolbox   
%      
    d1 = 0:1;
